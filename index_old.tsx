...
const logoUrl =
  'https://storage.googleapis.com/imageonline/ChatGPT%20Image%20Aug%2010%2C%202025%2C%2010_03_53%20AM.png';
...
const AppHeader: FC<{
  ...
  onSignOut: () => void;
}> = ({ user, currentView, setView, onSignOut }) => {
...
const ProfilesView: FC<{
  ...
  loading: boolean;
}> = ({ profiles, onSelectProfile, loading }) => {
  if (loading) {
    return (
            ...
            {audits.map((audit) => (
              <div key={audit.id} className="audit-item-card">
                <div
                  ...
                  role="button"
                  aria-expanded={selectedAudit === audit.id}
                >
...
const ToolsView: FC = () => {
  ...
  const handleGeneratePost = async () => {
    ...
    if (!geminiProxy) {
      setPostError('AI is unavailable');
      return;
    }
  ...
  const handleGetIdeas = async () => {
    ...
    if (!geminiProxy) {
      setKeywordError('AI is unavailable');
      return;
    }
...
const App: FC = () => {
  ...
  const fetchProfiles = useCallback(async (currentUser: User | null) => {
    ...
    try {
      const q = db
        .collection('clients')
        .where('consultant_uid', '==', currentUser.uid)
        .orderBy('createdAt', 'desc');
      const querySnapshot = await q.get();
      const profilesData = querySnapshot.docs.map(
        (doc) =>
          ({
            id: doc.id,
            ...doc.data(),
  ...
  useEffect(() => {
    ...
    const unsubscribe = auth.onAuthStateChanged((currentUser) => {
      if (isMounted) {
        console.log(
          'Auth state changed:',
          currentUser ? `Signed in as ${currentUser.displayName}` : 'Signed out'
        );
        setUser(currentUser);
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'AUDIT':
        return <AuditView business={auditTarget} onSaveAudit={handleSaveAudit} />;
      case 'PROFILES':
        return (
          <ProfilesView
            profiles={profiles}
      ...
      case 'TOOLS':
        return <ToolsView />;
      default:
        return <MapView onStartAudit={handleStartAudit} />;
    }
  };
